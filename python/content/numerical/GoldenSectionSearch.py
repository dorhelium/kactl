...
discrete version.
Usage:
    def func(x): return 4 + x + 0.3 * x * x
    xmin = gss(-1000, 1000, func)
...
def gss(a, b, f, eps=1e-7):
    """
    Golden section search for minimum of unimodal function f on [a, b]
    Change f1 < f2 to f1 > f2 to find maximum
    """
    # It is important for r to be precise, otherwise we don't necessarily
    # maintain the inequality a < x1 < x2 < b.
    r = (math.sqrt(5) - 1) / 2
    x1 = b - r * (b - a)
    x2 = a + r * (b - a)
    f1 = f(x1)
